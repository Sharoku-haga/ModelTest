...
// マウスカーソルの座標をセットする関数
void InputMouse::SetMouseCursorPos(float posX, float posY)
{
	...
	if( currentWnd == m_hWnd )					// ゲームウィンドウがアクティブかどうか
	{
		...
		int mousePosX = static_cast<int>(posX) + static_cast<int>(rect.left);
		int mousePosY = static_cast<int>(posY) +  static_cast<int>(rect.top);
